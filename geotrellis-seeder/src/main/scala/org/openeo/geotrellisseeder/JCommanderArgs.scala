...
class JCommanderArgs {
  ...
  @Parameter(names = Array("--selectOverlappingTile"), required = false, description = "use same values from same tile when there is overlap instead of taking max of 2 values", converter = classOf[StringOptionConverter])
