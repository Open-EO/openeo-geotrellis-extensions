...
void build(skipTests = false, skipSentinelHubTests = false){
    ...
    docker.image(env.MAVEN_IMAGE).inside('-v /var/run/docker.sock:/var/run/docker.sock -v /home/jenkins/.m2:/localdata/M2 -v /etc/hadoop/conf:/etc/hadoop/conf:ro -v /data:/data:ro -u root') {
        withEnv(jdkEnv) {
