...
void build(skipTests = false, skipSentinelHubTests = false){
    ...
    docker.image(env.MAVEN_IMAGE).inside('-v /var/run/docker.sock:/var/run/docker.sock -v /home/jenkins/.m2:/root/.m2 -v /etc/hadoop/conf:/etc/hadoop/conf:ro -v /data:/data:ro -u root') {
        withEnv(jdkEnv) {
            ...
            try {
                withCredentials([
                    ...
                    buildInfo = rtMaven.run pom: 'pom.xml', goals: '-P default,wmts,integrationtests -U -Dmaven.repo.local=/root/.m2 clean install' + rtMaven.opts
                    try {
