  ...
  def run(processGraph: File): Unit = {
    ...
    val cmd =
      if (debug) {
        ...
        f"docker run -e ZOOKEEPERNODES=192.168.207.56,192.168.207.57,192.168.207.58 -e LD_LIBRARY_PATH=/opt/venv/lib/python3.11/site-packages/jep -p $debugPort:5005 -p $sparkUIPort:4040 $credentialsFileMapping $optionalDataMapping $optionalEODataMapping -v $outputDir:/out -v $hostGraphFolder:/graphs $classPathMappings $dockerImage /graphs/$processGraphName /out $dockerClassPath DEBUG"
      } else {
        f"docker run -e ZOOKEEPERNODES=192.168.207.56,192.168.207.57,192.168.207.58 -e LD_LIBRARY_PATH=/opt/venv/lib/python3.11/site-packages/jep -v $outputDir:/out $credentialsFileMapping $optionalDataMapping $optionalEODataMapping -v $hostGraphFolder:/graphs $classPathMappings $dockerImage /graphs/$processGraphName /out $dockerClassPath"
      }
