...
class NetCDFRDDWriterTest extends RasterMatchers{
  ...
  def testNetCDFBandAttributes(): Unit = {
    def setup(cellType:CellType) = {
      ...
      Assert.assertNotNull(b1.findAttribute("raster:scale"))
      Assert.assertNotNull(b1.findAttribute("raster:offset"))
      ...
      Assert.assertNull(b2.findAttribute("raster:scale"))
      Assert.assertNull(b2.findAttribute("raster:offset"))
