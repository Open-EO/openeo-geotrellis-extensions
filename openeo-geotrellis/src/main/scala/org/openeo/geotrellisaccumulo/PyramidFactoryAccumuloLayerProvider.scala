...
class PyramidFactoryAccumuloLayerProvider(val layerName: String)(implicit accumuloSupplier: () => AccumuloInstance) extends LayerProvider {
  ...
  private def readLayer(from: ZonedDateTime, to: ZonedDateTime, boundingBox: ProjectedExtent = null, zoom: Int, sc: SparkContext): MultibandTileLayerRDD[SpaceTimeKey] = {
    val (pyramidFactory, metadata) = readCache.getUnchecked((zoom, sc))
  ...
  override def readMetadata(zoom: Int, sc: SparkContext): TileLayerMetadata[SpaceTimeKey] = {
    readCache.getUnchecked((zoom, sc))._2
  }
  ...
  private val readCache: LoadingCache[(Int, SparkContext), (PyramidFactory, TileLayerMetadata[SpaceTimeKey])] = {
    CacheBuilder.newBuilder()
      .expireAfterWrite(15, TimeUnit.MINUTES)
      .build(
        new CacheLoader[(Int, SparkContext), (PyramidFactory, TileLayerMetadata[SpaceTimeKey])] {
          def load(key: (Int, SparkContext)): (PyramidFactory, TileLayerMetadata[SpaceTimeKey]) = {
            ...
            val pyramidSeq = pyramidFactory.pyramid_seq(layerName, LatLng.worldExtent, "EPSG:4326")
            val metadata = pyramidSeq.find(_._1 == key._1).getOrElse(pyramidSeq.sortBy(_._1).reverse.head)._2.metadata
