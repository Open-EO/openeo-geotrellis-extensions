...
class CatalogApiTest {
  ...
  def dateTimes(): Unit = {
    val dateTimes = catalogApi.dateTimes(
      ...
      ProjectedExtent(Extent(35.666439, -6.23476, 35.861576, -6.075694), LatLng),
      from = LocalDate.of(2021, 2, 1).atStartOfDay(utc),
      to = LocalDate.of(2021, 2, 17).atStartOfDay(utc),
      accessToken
    )
    ...
    val expectedDates = Seq(LocalDate.of(2021, 2, 3))
